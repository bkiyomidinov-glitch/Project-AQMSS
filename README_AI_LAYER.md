...
### 2. Configure Input Price CSV (required)
Use one of the options:

- CLI: pass `--price-csv` to scripts that need raw candles
- ENV: set `AMQSS_PRICE_CSV`
- Legacy fallback: if not provided, scripts try `~/Downloads/EURUSD_60_2025-01-20_2026-01-19.csv`

Example (PowerShell):
```powershell
$env:AMQSS_PRICE_CSV="C:\path\to\EURUSD_60.csv"
```

### 3. Optional Global Paths (portable defaults already set)
If not set, defaults are project-relative:
...
Generate scores:
```bash
python main.py --price-csv "C:\path\to\EURUSD_60.csv"
```

Build ML dataset:
```bash
python dataset_builder.py --amqss-csv results/market_scores.csv --price-csv "C:\path\to\EURUSD_60.csv" --output-csv data/dataset.csv
```

Train model:
```bash
python trainer.py --dataset-csv data/dataset.csv --models-dir models
```

Evaluate model:
```bash
python evaluator.py --models-dir models --dataset-csv data/dataset.csv --output-dir models
```

Run main with trained AI:
```bash
python main.py --price-csv "C:\path\to\EURUSD_60.csv" --models-dir models --results-dir results
```

### 5. One-Command Pipeline
```bash
python run_pipeline.py --price-csv "C:\path\to\EURUSD_60.csv"
```

Optional tuning:
```bash
python run_pipeline.py --price-csv "C:\path\to\EURUSD_60.csv" --test-fraction 0.2 --n-estimators 150
```
